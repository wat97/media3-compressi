...
class CompressionPolicyEngineTest {
    ...
    fun `prefers hevc on api 34 when encoder exists and source safe`() {
        ...
        val codec = engine.chooseCodec(source, capability)
    ...
    fun `balanced hevc bitrate lower than avc`() {
        ...
        val hevc = engine.chooseTargetBitrate(source, OutputCodec.HEVC, CompressionPreset.BALANCED, null)
        val avc = engine.chooseTargetBitrate(source, OutputCodec.AVC, CompressionPreset.BALANCED, null)
